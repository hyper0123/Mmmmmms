...
def main():
    ...
    for idx, item in enumerate(data, start=1):
        try:
            ...
            # En caso de error, garantizar placeholder y continuar
            enriched.append({
                'name': 'Variado',
                'samples': [{
                    'name': item.get('samples', [{}])[0].get('name', ''),
                    'url': item.get('samples', [{}])[0].get('url', ''),
                    'icono': '',
                    'iconoHorizontal': '',
                    'iconpng': '',
                    'type': 'PELICULA',
                    'descripcion': '',
                    'anio': item.get('samples', [{}])[0].get('anio', ''),
                    'genero': [],
                    'duracion': ''
                }]
            })
        finally:
            print(f"Processed {idx}/{total}: {orig_title}")
